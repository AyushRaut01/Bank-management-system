...
@WebServlet("/block")
public class Block extends GenericServlet {

    @Override
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        Long AccountNumber = Long.parseLong(req.getParameter("acc"));
        try {
            PreparedStatement ps = LoginVerification.conn.prepareStatement("update account set status = 0 where account_number = ?");
